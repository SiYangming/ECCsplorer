...
> **状态：计划已完善，待批准后执行（当前不执行任何改动）**
> 已确认决策：镜像只构建 amd64（覆盖 arm64 版本）；conda 包推送 anaconda.org；genrich 用 nf-core 官方模块替换 local/genrich；**cresil 不做任何修改**（维持现状，不拆分、不执行）；**新增任务：完善 tidehunter 模块支持 README 全部用法**；修正：**haarz 是 segemehl 套件的附属程序（非独立工具）**；**新增：RepeatExplorer2（seqclust）聚类模块（ECCsplorer 聚类核心依赖，当前缺失）**；**haarz 模块目录迁移到 modules/local/segemehl/haarz/**。
...
- [ ] 新建 `modules/local/eccsplorer_slim/seqclust/main.nf`（进程 `ECCSPLORER_SEQCLUST`）：
  ...
  - **conda 包重建（build 1）方案（用户执行，2026-08-19 确认）**：① 新增 `patch/r2py_rserve_retry.py`（幂等：已含重试则跳过，30×1s）② 复制为 `recipe/patch_r2py.py`，`recipe/build.sh` 在 `cp -a lib...` 后调用 `python "${RECIPE_DIR}/patch_r2py.py" "${REXDIR}/lib/r2py.py"` ③ wrapper 在 `cd "${REX_DIR}"` 后加 REPEX_DATABASES 软链：`[ -n "${REPEX_DATABASES:-}" ] && [ ! -d "${REX_DIR}/databases" ] && ln -s "${REPEX_DATABASES}" "${REX_DIR}/databases"` ④ `meta.yaml` build: 0→1 ⑤ `conda build recipe/` + `anaconda upload`（yangmingsi）⑥ 服务器验证：conda 环境 + `export REPEX_DATABASES=/data1/users/siyangming/PublicDB/RepeatExplorer` + seqclust 小样本
...
- [ ] **Rserve 启动竞态 patch（必做，已实测确认）**：镜像 2.3.8 的 `lib/r2py.py` 中 `time.sleep(1)` 后立即连接 Rserve，启动稍慢即 `ConnectionResetError`（mac 与服务器均复现；手动启动 Rserve + pyRserve 连接正常，确认是竞态非环境问题）。模块脚本运行前需对镜像内 `/repex_tarean/lib/r2py.py` 执行 patch：把 `sleep(1)+单次连接` 改为 `30 次重试循环`（已实测有效）。**conda 模式**：实施时确认 `yangmingsi::repeatexplorer==2.3.8` 包内 `lib/r2py.py` 是否已含重试 patch——若已固化则 conda 模式无需运行时 patch；若未固化，运行时 patch 或重新构建 conda 包
...
- [ ] **双模式验证**：conda 模式（build 1 包）与 docker 模式（镜像）各跑小样本 seqclust，确认输出一致
...
8. **RepeatExplorer2（seqclust）**：ECCsplorer 聚类核心依赖，官方 nf-core 库无此模块；**新增本地模块 `modules/local/eccsplorer_slim/seqclust/`** 补齐 clu 链路（clu_prepare → seqclust → clu_candidates）；运行环境双模式：docker 镜像 **`docker.1ms.run/kavonrtep/repeatexplorer:2.3.8`**（已实测）+ **conda 包已构建**（`yangmingsi::repeatexplorer==2.3.8` + `yangmingsi::mgblast==2.2.14`，用户确认）；模块需处理三个已确认问题：Rserve 启动竞态（patch r2py.py，conda 包内是否固化待确认）、输入 ≥1000 条、comparative 模式需样本前缀（详见 B8 实测结论）
